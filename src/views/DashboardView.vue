<template>
  <div class="px-4 py-5 space-y-5" ref="containerRef">
    ...
    <!-- Header App Bar -->
    <div class="flex items-center justify-between dashboard-header opacity-0 translate-y-[-20px]">
    ...
    <div v-if="store.currentUser" class="gradient-overlay rounded-3xl p-5 text-white shadow-xl glow-primary space-y-4 user-welcome-card opacity-0 scale-95 relative overflow-hidden">
      ...
      <div class="flex items-start justify-between">
        <div>
          ...
          <h3 class="text-xl font-bold mt-0.5">{{ store.currentUser.name }}</h3>
          <p class="text-xs text-teal-100/80 flex items-center mt-1">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3 mr-1 text-amber-300" viewBox="0 0 20 20" fill="currentColor">
              <path fill-rule="evenodd" d="M5.05 4.05a7 7 0 119.9 9.9L10 18.9l-4.95-4.95a7 7 0 010-9.9zM10 11a2 2 0 100-4 2 2 0 000 4z" clip-rule="evenodd" />
            </svg>
            {{ store.currentUser.address }}
          </p>
        </div>
        <span class="px-2 py-0.5 bg-white/20 text-white rounded text-[10px] font-bold uppercase tracking-wider">Shohibul</span>
      </div>

      <!-- Payment Month Status -->
      <div class="bg-white/10 rounded-2xl p-3 flex justify-between items-center backdrop-blur-sm">
        <div>
          <p class="text-[10px] text-teal-100">Setoran Terakhir</p>
          <p class="text-sm font-bold text-white">{{ store.currentUser.lastPaymentMonth }}</p>
        </div>
        <div class="flex items-center space-x-2">
          <span class="text-sm font-extrabold text-amber-300">Rp25.000</span>
          <span class="px-2 py-0.5 bg-green-400 text-teal-900 rounded-full text-xs font-bold">Lunas</span>
        </div>
      </div>
    </div>
    ...
    <div class="grid grid-cols-3 gap-3 stats-grid">
      <div class="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700/50 rounded-2xl p-3 text-center shadow-sm hover:shadow-md transition card-item opacity-0">
    ...
    <div class="bg-emerald-900 text-white rounded-3xl p-5 shadow-xl glow-primary space-y-4 progress-widget opacity-0 translate-y-[30px]">
      ...
      <div class="pt-2 border-t border-emerald-800 flex space-x-3">
    ...
    <div class="space-y-3 recent-payments-list opacity-0 translate-y-[30px]">
      <div class="flex justify-between items-center">
        <h4 class="text-sm font-bold text-gray-800 dark:text-white flex items-center">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1 text-primary dark:text-primary-light" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div class="space-y-2 max-h-[260px] overflow-y-auto pr-1">
        <div 
          ...
          :key="tx.id"
          class="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700/50 rounded-2xl p-3 flex justify-between items-center shadow-sm hover:shadow-md transition"
        >
          <div class="flex items-center space-x-3">
            <div 
...
onMounted(() => {
  ...
  const ctx = gsap.context(() => {
    ...
    tl.to('.dashboard-header', { opacity: 1, y: 0, duration: 0.6 })
      .to('.user-welcome-card', { opacity: 1, scale: 1, duration: 0.6 }, '-=0.4')
      .to('.card-item', { opacity: 1, stagger: 0.1, duration: 0.5 }, '-=0.4')
      .to('.progress-widget', { opacity: 1, y: 0, duration: 0.6 }, '-=0.3')
      .to('.recent-payments-list', { opacity: 1, y: 0, duration: 0.6 }, '-=0.4')
  }, containerRef.value)
})
</script>
